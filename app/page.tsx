    ...
    <div className="min-h-screen bg-[#0a0e17] text-[#e2e8f0]">
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled ? 'bg-[#0a0e17]/95 backdrop-blur-md border-b border-[#1f2937]' : 'bg-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            {NAV_LINKS.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="hidden sm:block text-xs font-semibold tracking-[0.15em] text-[#94a3b8] hover:text-[#e2e8f0] transition-colors"
              >
                {link.label}
              </a>
            ))}
          ...
          <Link
            href="/auth"
            className="text-xs font-semibold tracking-wider px-4 py-2 rounded border border-[#1f2937] text-[#e2e8f0] hover:bg-[#1f2937]/50 transition-colors"
          >
            LOG IN
          </Link>
        </div>
      ...
      <section id="hero" className="relative min-h-screen flex items-end justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 pb-16 sm:pb-24 text-center">
          ...
          {/* Subtitle */}
          <p className="mt-6 text-sm sm:text-base font-semibold tracking-[0.2em] text-[#94a3b8] uppercase">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            {FEATURE_TAGS.map((tag) => (
              <span
                key={tag}
                className="text-[11px] sm:text-xs font-semibold tracking-wider text-[#94a3b8] border border-[#1f2937] rounded px-3 py-1.5 bg-[#0a0e17]/50 backdrop-blur-sm"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="mt-10">
            <Link
              href="/auth"
              className="inline-block text-sm sm:text-base font-bold tracking-[0.15em] uppercase px-8 py-4 border-2 border-[#94a3b8]/40 text-[#e2e8f0] hover:bg-[#e2e8f0]/10 hover:border-[#e2e8f0]/60 transition-all duration-300 rounded"
            >
        ...
        <div ref={howFade.ref} className={howFade.className}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-16">
              <p className="text-[10px] font-semibold tracking-[0.3em] text-[#6366f1] uppercase mb-3">How It Works</p>
              <h2 className="text-3xl sm:text-4xl font-extrabold text-[#e2e8f0]">Three Steps to Total Privacy</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {STEPS.map((step) => (
                <div key={step.num} className="relative bg-[#111827] border border-[#1f2937] rounded-xl p-8 card-glow">
                  {/* Step number */}
                  <span className="text-5xl font-extrabold font-mono text-[#1f2937] absolute top-4 right-4">{step.num}</span>
      ...
      {/* ========== FEATURE GRID ========== */}
      <section id="features" className="py-24 sm:py-32 bg-[#0d1117]">
        <div ref={featureFade.ref} className={featureFade.className}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-16">
              <p className="text-[10px] font-semibold tracking-[0.3em] text-[#6366f1] uppercase mb-3">Features</p>
              <h2 className="text-3xl sm:text-4xl font-extrabold text-[#e2e8f0]">Your Privacy Arsenal</h2>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {FEATURES.map((f) => (
                <div key={f.title} className="bg-[#111827] border border-[#1f2937] rounded-xl p-6 card-glow group">
      ...
      <section id="pricing" className="py-24 sm:py-32">
        <div ref={pricingFade.ref} className={pricingFade.className}>
          <div className="max-w-5xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-16">
              <p className="text-[10px] font-semibold tracking-[0.3em] text-[#6366f1] uppercase mb-3">Pricing</p>
              <h2 className="text-3xl sm:text-4xl font-extrabold text-[#e2e8f0]">Simple, Transparent Pricing</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
              {/* Free Tier */}
              <div className="bg-[#111827] border border-[#1f2937] rounded-xl p-8 card-glow">
                <div className="mb-6">
                ...
                <Link
                  href="/auth"
                  className="block text-center mt-8 px-6 py-3 rounded-lg border border-[#1f2937] text-sm font-semibold text-[#e2e8f0] hover:bg-[#1f2937]/50 transition-colors"
                >
                  Get Started Free
                </Link>
              </div>
              ...
              {/* Pro Tier */}
              <div className="bg-[#111827] border-2 border-[#6366f1]/40 rounded-xl p-8 card-glow relative">
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-[10px] font-bold tracking-wider uppercase px-3 py-1 rounded-full bg-[#6366f1] text-white">
                  Recommended
                </span>
                <div className="mb-6">
                  <h3 className="text-lg font-bold text-[#e2e8f0]">Pro</h3>
                  <div className="flex items-baseline gap-1 mt-2">
                    <span className="text-4xl font-extrabold font-mono text-[#6366f1]">$9.99</span>
                    <span className="text-sm text-[#64748b]">/month</span>
                  </div>
                ...
                <Link
                  href="/auth"
                  className="block text-center mt-8 px-6 py-3 rounded-lg bg-[#6366f1] hover:bg-[#818cf8] text-sm font-semibold text-white transition-colors"
                >
                  Start Pro Trial
                </Link>
              </div>
            ...
            <div className="mt-8 text-center">
              <p className="text-xs text-[#64748b]">
                ...
                Extra phone number <span className="font-mono text-[#e2e8f0]">$2.99</span>/mo
                {' '}&middot;{' '}
                Extra 10 aliases <span className="font-mono text-[#e2e8f0]">$1.99</span>/mo
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 sm:py-32 bg-[#0d1117]">
        <div ref={compFade.ref} className={compFade.className}>
          <div className="max-w-5xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-16">
              <p className="text-[10px] font-semibold tracking-[0.3em] text-[#6366f1] uppercase mb-3">Comparison</p>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#1f2937]">
                      ...
                      {COMPETITORS.map((c, i) => (
                        <th key={c} className={`text-center px-3 py-3 text-[10px] font-semibold tracking-wider uppercase min-w-[100px] ${i === 0 ? 'text-[#6366f1]' : 'text-[#64748b]'}`}>
                          {c}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {COMP_FEATURES.map((f, ri) => (
                      <tr key={f.name} className={`border-b border-[#1f2937]/50 last:border-0 ${ri % 2 === 1 ? 'bg-[#0d1321]' : ''}`}>
                        <td className="px-4 py-3 text-sm text-[#e2e8f0] font-medium">{f.name}</td>
                        {f.vals.map((v, ci) => (
                          <td key={ci} className="text-center px-3 py-3">
                            {v ? (
                              <svg className={`w-5 h-5 mx-auto ${ci === 0 ? 'text-[#22c55e]' : 'text-[#22c55e]/60'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                              </svg>
                            ) : (
                              <svg className="w-5 h-5 mx-auto text-[#64748b]/40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                              </svg>
                            )}
                          </td>
      ...
      {/* ========== FOOTER ========== */}
      <footer id="footer" className="border-t border-[#1f2937] py-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex items-center justify-center gap-6 mb-8">
            <a href="#" className="text-xs text-[#64748b] hover:text-[#e2e8f0] transition-colors">Privacy Policy</a>
            <span className="text-[#1f2937]">&middot;</span>
            <a href="#" className="text-xs text-[#64748b] hover:text-[#e2e8f0] transition-colors">Terms</a>
            <span className="text-[#1f2937]">&middot;</span>
            <a href="https://github.com/JeetVaidya1/PhantomShield" target="_blank" rel="noopener noreferrer" className="text-xs text-[#64748b] hover:text-[#e2e8f0] transition-colors">
              GitHub
            </a>
          </div>
          ...
          {/* Attribution */}
          <p className="text-xs text-[#64748b]">
            Built by <a href="https://github.com/JeetVaidya1" target="_blank" rel="noopener noreferrer" className="text-[#94a3b8] hover:text-[#e2e8f0] transition-colors">Jeet Vaidya</a>
          </p>
        </div>
      </footer>
    </div>
